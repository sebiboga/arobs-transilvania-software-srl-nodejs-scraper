...
describe('solr.js', () => {
  ...
  describe('querySOLR', () => {
    it('should return response object with docs', async () => {
      mockFetch.mockResolvedValue(makeSolrResponse(2, [
        { id: 'job1', url: 'https://careers-arobs.icims.com/jobs/3444/job', cif: '11291045' },
        { id: 'job2', url: 'https://careers-arobs.icims.com/jobs/3445/job', cif: '11291045' }
      ]));

      const result = await solr.querySOLR('11291045');
    ...
    it('should throw when SOLR_AUTH is missing', async () => {
      delete process.env.SOLR_AUTH;
      await expect(solr.querySOLR('11291045')).rejects.toThrow('SOLR_AUTH not set in environment');
      process.env.SOLR_AUTH = 'test:test';
    });
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(solr.querySOLR('11291045')).rejects.toThrow('SOLR query error: 500');
    });
  });
  ...
  describe('queryCompanySOLR', () => {
    it('should return company data', async () => {
      mockFetch.mockResolvedValue(makeSolrResponse(1, [
        { id: '11291045', company: 'AROBS TRANSILVANIA SOFTWARE S.A.', brand: 'AROBS' }
      ]));

      const result = await solr.queryCompanySOLR('id:11291045');

      expect(result.numFound).toBe(1);
      expect(result.docs[0].brand).toBe('AROBS');
    });
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(solr.queryCompanySOLR('id:11291045')).rejects.toThrow('SOLR company query error: 401');
    });
  });
  ...
  describe('upsertJobs', () => {
    it('should accept array of jobs', async () => {
      ...
      const testJob = {
        url: 'https://careers-arobs.icims.com/jobs/3444/supplier-audit/job',
  ...
  describe('deleteJobByUrl', () => {
    it('should delete a job by URL', async () => {
      ...
      await expect(solr.deleteJobByUrl('https://careers-arobs.icims.com/jobs/old/job')).resolves.not.toThrow();
    });
  ...
  describe('deleteJobsByCIF', () => {
    it('should delete all jobs for a CIF', async () => {
      ...
      await expect(solr.deleteJobsByCIF('11291045')).resolves.not.toThrow();
    });

    it('should throw on HTTP error', async () => {
      mockFetch.mockResolvedValue(makeErrorResponse(500, 'Error'));

      await expect(solr.deleteJobsByCIF('11291045')).rejects.toThrow('SOLR delete error: 500');
    });
  });
  ...
  describe('Data Integrity', () => {
    it('should not have duplicate URLs for same CIF', async () => {
      mockFetch.mockResolvedValue(makeSolrResponse(2, [
        { url: 'https://careers-arobs.icims.com/jobs/3444/job', title: 'Supplier Audit', cif: '11291045' },
        { url: 'https://careers-arobs.icims.com/jobs/3445/job', title: 'Cloud PM', cif: '11291045' }
      ]));
    ...
    it('should have valid status values', async () => {
      ...
      mockFetch.mockResolvedValue(makeSolrResponse(3, [
        { url: 'https://careers-arobs.icims.com/jobs/3444/job', title: 'Supplier Audit', cif: '11291045', status: 'scraped' },
        { url: 'https://careers-arobs.icims.com/jobs/3445/job', title: 'Cloud PM', cif: '11291045', status: 'verified' },
        { url: 'https://careers-arobs.icims.com/jobs/3446/job', title: 'Architect', cif: '11291045', status: 'published' }
      ]));
