...
const COMPANY_NAME = "AROBS TRANSILVANIA SOFTWARE S.A.";
...
async function getJobs() {
  ...
  while (true) {
    const res = await fetch(
      ...
      { headers: { origin: "https://peviitor.ro", referer: "https://peviitor.ro/" } }
    );
...
async function main(args) {
  ...
  console.log("AROBS Job Validator");
  console.log("=".repeat(50));
  ...
  let active = 0, expired = 0, errors = 0;
  const expiredJobs = [];
  ...
  for (let i = 0; i < jobs.length; i++) {
    ...
    if (result.ok) {
      console.log(`OK ${job.job_title.substring(0, 50)}`);
      active++;
    } else if (result.status === 404 || result.status === 0) {
      console.log(`XX EXPIRED (${result.status}) - ${job.job_title.substring(0, 40)}`);
      console.log(`   URL: ${job.url}`);
      expiredJobs.push(job);
      expired++;
    } else {
      console.log(`!! STATUS ${result.status} - ${job.job_title.substring(0, 40)}`);
      errors++;
    }
  ...
  if (expired > 0) {
    ...
    if (!dryRun) {
      ...
      for (const job of expiredJobs) {
        ...
        if (ok) {
          console.log(`Deleted: ${job.job_title}`);
          deleted++;
        } else {
      ...
      console.log(`\nDry run - no jobs deleted. Run with --delete to actually remove.`);
    }
  }
